	...
	<div class="btn_area">
		<input type="button" class="btn_2" onclick="go_list()" value="목록"/>
			<sec:authentication property="principal" var="pinfo"/>
				<sec:authorize access="isAuthenticated()">
					<c:if test="${pinfo.username eq todo.t_username}">
						<input type="button" class="btn_3" onclick="list_delete()" value="삭제"/>
...
<script>
	function list_delete(){
		...
		listForm.action = "/tdam/todo/delete?t_seq=";
		listForm.submit();
			
	}
	function list_update(){
		var listForm = document.getElementById("listForm");
		listForm.action = "/tdam/todo/insertFrm";
		listForm.submit();
	}
	...
	function go_list(){
		...
		console.log(link);
		var listForm = document.getElementById("listForm");
		if(link != "http://localhost:8080/tdam/user/view?t_seq=${todo.t_seq}"){
			listForm.action = "/tdam/todo";
		} else {
			listForm.action = "/tdam/user/mypage";
		}
		listForm.submit();
	}
