...
<script>
	...
	function list_write2(){
		...
		listForm.action = "/tdam/todo/insertFrm2";
		listForm.submit();
	}
					...
					<tbody>
						<c:forEach items="${TODOS}" var="TODO" varStatus="status">
							<tr onclick="listView('${TODO.t_seq}')">
								<td>${PAGE.totalCount - ((PAGE.currentPageNo-1) * 10 + status.index)}</td>
								<td>${TODO.t_place}</td>
								<td>${TODO.t_title}</td>
					...
					<tbody>
						<c:forEach items="${TODOS2}" var="TODO" varStatus="status">
							<tr onclick="listView2('${TODO.t_seq2}')"> 
								<td>${PAGE2.totalCount - ((PAGE2.currentPageNo-1) * 10 + status.index)}</td>
								<td>${TODO.t_place2}</td>
								<td>${TODO.t_title2}</td>
								<td>${TODO.t_username}</td>
								<td>${TODO.t_sdate}</td>
