...
<div class="todo_body ">
	<sec:authorize access="isAuthenticated()">
		<h1 class="w3-text-blue">제보</h1>
		<form id="listForm" method="POST" enctype="multipart/form-data">
		<c:if test="${todo.t_seq != '' && todo.t_seq != null}">
...
<script>
	function list_write() {
		 ...
		 console.log($("#t_seq").val());
 		 if($("#t_seq").val() == null || $("#t_seq").val() == ""){
			listForm.action = "/tdam/todo/insert";
			listForm.submit();
 		}else{
			listForm.action = "/tdam/todo/update";
			listForm.submit();
			var listConfirm = confirm("나의 목록에서 보기-확인\n전체 목록에서 보기-취소");
			if(listConfirm){
				location= "/tdam/user/mypage";
			} else {
				location= "/tdam/todo";
			}
		}
	...
	function go_list(){
		...
		if(link == "http://localhost:8080/tdam/todo/insertFrm?t_seq=" || 
			link == "http://localhost:8080/tdam/todo/insertFrm?t_seq=${todo.t_seq}"){
			location = "/tdam/todo";
		} else {
			location = "/tdam/user/mypage";
		}
